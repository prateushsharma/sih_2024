...
// Function to calculate deficient nutrients with percentage difference
...
const fetchSoilType = async (lat, lon) => {
  try {
    ...
    if (jsonTopK.properties.probabilities.length >= 2) {
      ...
      // Calculate and show deficient nutrients
      const deficientNutrients = getDeficientNutrients(comparisonResult);
      console.log('Deficient Nutrients:', JSON.stringify(deficientNutrients, null, 2));
    } else {
